...
Original file is located at
    https://colab.research.google.com/drive/1Vl2tc8JibKtDAZod5hyF-WDNOxtcdO8a
"""
...
for i in range(Y_train.shape[1]):
  if(d_sgd["Y_prediction_train"][:,i]!=Y_train[:,i]):
    plt.imshow(X_train[:,i].reshape((28, 28)))
    show()

for i in range(Y_test.shape[1]):
  if(d_sgd["Y_prediction_test"][:,i]!=Y_test[:,i]):
    plt.imshow(X_test[:,i].reshape((28, 28)))
    show()

print("First set of images are the wrongly classified images in the training dataset and the second set of images are wrongly classified in test dataset. It can be inferred from these images that some of the numbers are so badly written that it might be confusing to infer them even through eye. Other were the cases where the shape of written letter was very much similar to the complement number")